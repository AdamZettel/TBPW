...
!    AtomsInit    		reads information on atom positions and types
!                   	  allocates the space for the dynamic variables
!    AtomsDestroy		deallocates space for the dynamic variables
! 
...
!
  ...
  type AtomBasisT  
    ...
    character*2,     pointer     :: elementLabel(:)
  end type AtomBasisT
...
!*************************************************************************
    ...
    allocate(atomBasis%numAtomsOfSpecies(atomBasis%numSpecies), &
           & atomBasis%elementLabel(atomBasis%numSpecies), stat = error)
    if(error /= 0) stop 'Error allocating atom space in LatticeInit.'
    ...
    if(error .eq. 0) then
      ...
      do i = 1, atomBasis%numSpecies
        ...
        read(unit = tagHandler%fileno, fmt = *, iostat = error) j, k, atomBasis%elementLabel(i)
        if(error /= 0) stop 'Error reading ChemicalSpeciesLabel.'
        print *, j, ' ', k, ' ', atomBasis%elementLabel(i)
    ...
    if(error /= 0) stop 'Error allocating atoms in LatticeInit.'
    ...
    allocate(atomBasis%atomCart(ndim, atomBasis%totalNumAtoms), &
           ...
           & stat = error)
    if(error /= 0) stop 'Error allocating atomCart in LatticeInit.'
    ...
    if(error /= 0) stop 'Error deallocating in LatticeInit.'
...
!  Deallocate space for atomic positions and infromation
    ...
    deallocate(atomBasis%numAtomsOfSpecies, atomBasis%elementLabel, &
             & atomBasis%speciesOfAtom, stat = error)
    if(error /= 0) stop 'Error deallocating numAtoms in AtomsDestroy.'

    deallocate(atomBasis%atomCart, atomBasis%atomLat, stat = error)
    if(error /= 0) stop 'Error deallocating atomCart in AtomsDestroy.'
