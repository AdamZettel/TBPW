...
!
!    gVectors(i,j)        j-th component (reciprocal lattice coordinates)
!                         of the i-th G-vector
!
...
!*************************************************************************
  Subroutine GVectorsInit( gVectors, structure1, tagHandler )
!*************************************************************************
!
...
!     EnergyCutOff
    ...
    if(error .eq. 0) then
      ...
      read(unit = tagHandler%fileno, fmt = '(G15.10)', iostat = error) gVectors%energyCutOff
    ...
    if(error .eq. 0) then
       ...
       print *, 'EnergyCutOff in Hartrees ', gVectors%energyCutOff
    endif
    ...
    ! The number of G vectors in the parallel piped constructed with
    ...
    ! Loop over all cells in the parallelpiped containing the sphere of 
    ...
    call StructureFactorInit( gVectors, structure1 )
...
! Deallocates the space allocated for the GVectors.
    ...
    deallocate(gVectors%gVectors, gVectors%kineticE, stat = error)
...
!*************************************************************************
  Subroutine StructureFactorInit( gVectors, structure1 )
!*************************************************************************
!
!   calculates the structure factors for each atom type at each G-vector
    ...
    real(double) :: arg 
    integer      :: iGVec, iElem , iAtom, error, atomI
    ...
    do iElem = 1, structure1%atomBasis%numSpecies
      ...
      do iAtom = 1, structure1%atomBasis%numAtomsOfSpecies(iElem)
